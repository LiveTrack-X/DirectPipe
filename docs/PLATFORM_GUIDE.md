...
|---|---|---|---|
| **Windows 10/11 x64** | 안정 대상 / Stable target | 로컬 Release 및 CI `windows-latest` ZIP | v4.2.8 후보는 로컬 Windows Release 등록 582개 중 580개 통과, 환경 의존 2개 건너뜀, 실패 0개입니다. 정확 태그 CI, 패키지 checksum, 실행 파일 버전 신원, 공개 자산 검증은 게시 게이트입니다. 신뢰 인증서가 없어 unsigned 상태를 명시합니다. 실기기·제3자 VST crash-containment는 별도 검증 범위입니다. |
...
> 현재 v4.2.8 후보 문서의 "지원"은 소스/CI 빌드 경로와 플랫폼 구현 존재를 의미합니다. 안정도와 실기기 검증 수준은 위 표의 상태를 따릅니다.
> In the current v4.2.8 candidate documentation, "supported" means the source/CI path and platform implementation exist. Release maturity and hardware validation follow the table above.

> **Windows 시작 복원**: 마지막으로 저장한 드라이버·입력·출력이 권위값입니다. 재부팅 후 목록 첫 장치나 시스템 기본 장치가 오류 없이 열려도 새 선택으로 채택하지 않고 저장 대상 복구를 계속하며, `CABLE Input`을 포함한 특정 장치명을 고정하지 않습니다. / **Windows startup restore**: the last saved driver, input, and output remain authoritative. An errorless first-enumerated or system-default endpoint is not adopted after reboot; recovery continues toward the saved targets, with no hard-coded device name including `CABLE Input`.
