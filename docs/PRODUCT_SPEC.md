...
| 폴백 보호 / Fallback Protection | 실제 loss가 먼저 보고된 경우에만 desired target과 다른 시작을 자동 폴백으로 처리한다. 선행 loss가 없는 유효한 시작은 v4.2.0과 같이 실제 기본 경로를 채택한다. `intentionalChange_`와 deferred generation은 intentional transaction 및 이후 설정 복원/수동 선택을 stale callback이 덮어쓰지 못하게 한다 / A desired-target mismatch is automatic fallback only after a real loss was published. A valid start with no preceding loss adopts the actual default route, matching v4.2.0. `intentionalChange_` and deferred generations keep stale callbacks from overwriting intentional transactions, later settings restores, or manual selections |
