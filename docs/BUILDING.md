...
> **플랫폼 지원 상태**: Windows 10/11 x64는 안정 릴리즈 대상입니다. v4.3.0 후보는 로컬 Windows Release 등록 582개 중 580개 통과, 환경 의존 2개 건너뜀, 실패 0개로 완료했습니다. 전체 플랫폼 빌드·패키지·checksum·Windows unsigned 상태는 정확 태그 CI에서 별도 검증합니다. 실기기·제3자 VST crash-containment는 별도 검증 범위입니다.
> **Platform support**: Windows 10/11 x64 is the stable release target. The v4.3.0 candidate completed 582 local Windows Release registrations with 580 passed, 2 environment-dependent skips, and 0 failures. Exact-tag CI separately verifies cross-platform builds, packages, checksums, and Windows unsigned state. Real-device and third-party VST crash-containment remain separate evidence.

## Support Status / 지원 상태

| Platform | Release status | CI target | Audio backend | Notes |
|---|---|---|---|---|
| Windows 10/11 x64 | Stable release / 안정 릴리즈 | `windows-latest` | WASAPI Shared/Low Latency/Exclusive, ASIO when SDK is available | v4.3.0 passed local Release validation; exact-tag CI and public assets remain the publication gate. CI requires unsigned binaries because no trusted signing secrets are configured; no real-device claim. |
...
세 개의 테스트 실행 파일을 빌드합니다: `directpipe-tests`(코어, JUCE 의존성 없음), `directpipe-host-tests`(JUCE 호스트 범위), `directpipe-endpoint-watcher-tests`(endpoint 알림 집중 검증). v4.3.0은 로컬 Windows Release 등록 582개 중 580개 통과, 환경 의존 2개 건너뜀, 실패 0개로 완료했습니다. 정확 태그 CI, 패키지 checksum, 실행 파일 버전 신원은 공개 전 게이트입니다. 신뢰 인증서가 설정되기 전 Authenticode는 선택 사항입니다.
