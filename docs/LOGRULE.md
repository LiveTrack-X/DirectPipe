...
```
[11:23:45.123] INF [APP] DirectPipe v4.2.6 started
...
- AUDIO audit snapshots include `latencyMs` as the PDC-inclusive total estimate used by the status bar/state, `procMs` as callback execution time only, `xruns60` as the current 60-second rolling display count, and `xrunDelta` as the monotonic device XRun events since the previous audit snapshot.
...
```
INF [APP] DirectPipe v4.2.6 started
...
**Purpose**: 재현에 필요한 사용자 환경의 기준 정보를 빠르게 확보. / Quickly captures baseline environment details needed for reproduction.
...
15. **Audio audit snapshot** — `StatusUpdater` emits ~1Hz `AUDIO`/`MONITOR` audit snapshots while Audit Mode is ON. `latencyMs` is the status/state total estimate including active-chain PDC; `procMs` is callback execution time only. Snapshots also include desired/actual devices, SR/BS, XRUN/callback-overrun deltas, monitor PLL/fill fields, monitor drop/underrun/emergency-trim deltas, and mute/lost flags. Keep this on the message thread; never log directly from RT callbacks.
