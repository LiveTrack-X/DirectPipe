...
- **ControlManager** — Owns and manages all external control subsystems (HotkeyHandler, MidiHandler, WebSocketServer, HttpApiServer). `initialize(bool enableExternalControls)` conditionally skips external control init for multi-instance audio-only mode. `externalControlsActive_` flag preserved across `reloadConfig()`/`applyConfig()`. / 모든 외부 제어 서브시스템 소유 및 관리. 다중 인스턴스 audio-only 모드를 위한 조건부 초기화.
