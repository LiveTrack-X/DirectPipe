...
- **SafetyLimiter** — RT-safe feed-forward limiter (0.1ms attack, 50ms release). Inserted after VSTChain, before all output paths. Atomic params: `enabled`, `ceilingdB`. GR feedback via atomic for UI. / RT 안전 피드포워드 리미터. VSTChain 이후, 모든 출력 경로 이전에 삽입. Atomic 파라미터.
...
- **BuiltinAutoGain** — LUFS-based automatic gain control (AudioProcessor subclass). K-weighting ITU-R BS.1770 sidechain. Incremental `runningSquareSum_`. Configurable target LUFS, attack/release, max gain. / LUFS 기반 자동 게인 제어 (AudioProcessor 서브클래스). K-weighting ITU-R BS.1770 사이드체인. 증분식 `runningSquareSum_`.
...
- **ActionHandler** — Centralized action event handling, extracted from MainComponent. Receives `ActionEvent` from `ActionDispatcher` and routes to AudioEngine, VSTChain, PresetManager, OutputRouter, etc. `doPanicMute(bool)` consolidates panic mute logic: saves pre-mute state (monitor, output mute, IPC), mutes everything, stops active recording. On unmute, restores previous state (recording does not auto-restart). All action cases check `engine_.isMuted()` to block during panic — only PanicMute/InputMuteToggle can change state. Callback-based decoupling from MainComponent (`onDirty`, `onNotification`, `onPanicStateChanged`, `onRecordingStopped`, etc.). / MainComponent에서 추출된 중앙 액션 이벤트 처리. `doPanicMute(bool)`가 패닉 뮤트 로직 통합: pre-mute 상태 저장, 전체 뮤트, 녹음 중지. 해제 시 이전 상태 복원 (녹음은 자동 재시작 안 함). 모든 액션이 `isMuted()` 체크로 패닉 중 차단.
...
- **HttpApiServer** — HTTP REST API (port 8766) for one-shot GET commands. CORS enabled with OPTIONS preflight support (`Access-Control-Allow-Methods: GET, POST, PUT, DELETE, OPTIONS`) for browser-based clients. 3-second read timeout. Input validation: volume/parameter values validated as numeric (rejects non-numeric strings), range-checked. Gain delta endpoint scales by 10× to compensate for ActionHandler's `*0.1f` hotkey step design. Proper HTTP status codes (404 Not Found, 405 Method Not Allowed, 400 Bad Request) — `processRequest` returns `pair<int, string>`. Recording toggle, plugin parameter, IPC toggle, MIDI test injection endpoints. / HTTP REST API. CORS preflight (OPTIONS) 지원. 3초 읽기 타임아웃. 입력 검증: 볼륨/파라미터 값 숫자 유효성 검사. 게인 델타는 ActionHandler의 `*0.1f` 스케일링 보정을 위해 10× 스케일링. 적절한 HTTP 상태 코드 (404/405/400).
...
- Panic mute remembers pre-mute state (monitor, output mute, IPC), restores on unmute. Active recording is stopped on panic engage (does not auto-restart). During panic mute, OUT/MON/VST buttons and all external controls (hotkey/MIDI/Stream Deck/HTTP) are locked — all actions (bypass, volume, preset, recording, gain, plugin parameters) are blocked. Only PanicMute/unmute can change state. / Panic Mute: pre-mute 상태(모니터, 출력, IPC) 기억/복원. 패닉 시 녹음 자동 중지 (해제 시 재시작 안 함). 패닉 중 모든 액션(바이패스, 볼륨, 프리셋, 녹음, 게인, 플러그인 파라미터) 차단.
...
8. Safety Limiter (feed-forward, in-place on workBuffer — before ALL output paths) / Safety Limiter (피드포워드, workBuffer 인플레이스 — 모든 출력 경로 전에 적용)
