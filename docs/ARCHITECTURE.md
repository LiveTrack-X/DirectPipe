...
- **StateBroadcaster** — Pushes AppState changes to all connected StateListeners as JSON. Thread-safe: state updates from any thread, listeners always notified on message thread via `callAsync`. Copy-before-iterate for reentrant safety. WebSocket broadcast on dedicated thread (non-blocking). / 상태 변경을 JSON으로 모든 리스너에 푸시. 스레드 안전: 어느 스레드에서든 상태 업데이트 가능, 리스너는 항상 메시지 스레드에서 callAsync로 알림. 재진입 안전을 위한 copy-before-iterate. WebSocket 브로드캐스트 전용 스레드 (비차단).
