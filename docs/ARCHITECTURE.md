...
- **ActionHandler** — Centralized action event handling, extracted from MainComponent. Receives `ActionEvent` from `ActionDispatcher` and routes to AudioEngine, VSTChain, PresetManager, OutputRouter, etc. / MainComponent에서 추출된 중앙 액션 이벤트 처리. ActionDispatcher로부터 ActionEvent를 받아 각 서브시스템으로 라우팅.
