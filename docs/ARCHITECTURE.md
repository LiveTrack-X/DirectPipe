...
15. **WebSocket broadcast mutex release** — `broadcastToClients` releases `clientsMutex_` before socket writes (prevents slow client blocking shutdown) and joins broadcast thread outside lock to prevent deadlock. / `broadcastToClients`가 소켓 쓰기 전에 `clientsMutex_`를 해제 (느린 클라이언트가 shutdown 차단 방지)하고 교착 방지를 위해 잠금 밖에서 스레드 join.
