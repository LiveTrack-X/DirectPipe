...
> **Windows**: 현재 신뢰된 코드 서명 인증서가 없어 v4.2.6도 unsigned일 수 있으며 SmartScreen/브라우저 평판 경고가 나타날 수 있습니다. 공식 GitHub Release에서만 받고 ZIP을 `checksums.sha256`과 대조하세요. 디지털 서명이 있는 빌드는 상태가 `Valid`인지도 확인하고, 손상되거나 유효하지 않은 서명은 실행하지 마세요.
> DirectPipe currently has no trusted Windows code-signing certificate, so v4.2.6 may be unsigned and trigger browser/SmartScreen reputation warnings. Download only from the official GitHub Release and verify the ZIP against `checksums.sha256`. If a signature is present, also require it to be `Valid`.
>
