...
3. **Input** — USB 마이크 선택
4. **Output** — 가상 오디오 케이블 선택 (예: Windows `VB-Cable`, macOS `BlackHole`, Linux `PipeWire`)
...
4. **Output** — Select a virtual audio cable (e.g., Windows `VB-Cable`, macOS `BlackHole`, Linux `PipeWire`)
...
| **설치 / Setup** | 가상 케이블 드라이버 설치 필요 (Windows: VB-Cable, macOS: BlackHole, Linux: PipeWire) / Install virtual cable driver | 플러그인 파일 복사만 하면 됨 (Windows: .dll, macOS: .vst, Linux: .so) / Just copy the plugin file |
...
**동시 사용 (권장)**: Discord는 가상 케이블로, OBS는 Receiver VST로 보내면 **각 앱의 마이크를 OUT/VST 버튼으로 독립 제어** 가능.

**Dual use (recommended)**: Send Discord via virtual cable, OBS via Receiver VST — independently control each app's mic with OUT/VST buttons.
