...
> **Version 3.10.2** — [GitHub Releases](https://github.com/LiveTrack-X/DirectPipe/releases)
...
1. [VB-Audio Virtual Cable](https://vb-audio.com/Cable/) 설치 (무료) → PC 재부팅 / Install (free) → reboot PC
2. DirectPipe Audio 탭: Output → `CABLE Input (VB-Audio Virtual Cable)` 선택 / Audio tab: Output → select `CABLE Input (VB-Audio Virtual Cable)`
3. Discord/Zoom 음성 설정: 마이크 → `CABLE Output (VB-Audio Virtual Cable)` 선택 / Voice settings: mic → select `CABLE Output (VB-Audio Virtual Cable)`

```
USB 마이크 → DirectPipe → VB-Cable Input
USB Mic   → DirectPipe → VB-Cable Input
                                ↓
              Discord/Zoom ← VB-Cable Output (마이크로 인식 / recognized as mic)
```

**OBS (Receiver VST2 — 가상 케이블 불필요 / no virtual cable needed):**
1. `DirectPipe Receiver.dll`을 아래 VST2 폴더 중 하나에 복사 / Copy to one of these VST2 folders:
...
| | VB-Cable (가상 케이블 / Virtual Cable) | Receiver VST (IPC) |
|---|---|---|
| **대상 앱 / Target apps** | Discord, Zoom, Teams 등 **마이크 입력을 선택하는 앱** / Apps that **select mic input** | OBS 등 **VST2 필터를 지원하는 앱** / Apps that **support VST2 filters** |
| **설치 / Setup** | VB-Cable 드라이버 설치 + PC 재부팅 필요 / Install VB-Cable driver + reboot | DLL 파일 복사만 하면 됨 / Just copy the DLL file |
| **원리 / How it works** | DirectPipe → 가상 케이블 → 앱이 "마이크"로 인식 / App recognizes as "mic" | DirectPipe → 공유 메모리 IPC → 앱 내 VST 필터가 수신 / VST filter receives via shared memory |
| **지연 / Latency** | VB-Cable 버퍼 + WASAPI 버퍼에 따라 다름 / Depends on VB-Cable + WASAPI buffers | 공유 메모리 IPC, 버퍼 설정에 따라 5~85ms / Shared memory IPC, 5~85ms depending on buffer |
| **장점 / Pros** | 어떤 앱이든 "마이크"로 인식 가능 / Any app can use it as "mic" | 가상 케이블 설치 불필요, 시스템 깔끔 / No virtual cable needed, clean system |
| **제한 / Cons** | 가상 오디오 드라이버 설치 필요 / Requires virtual audio driver | VST2 필터 지원 앱에서만 사용 가능 / Only in apps supporting VST2 filters |
...
Select the balance between latency and stability in the Receiver VST GUI **Buffer** dropdown.
...
|---|---|---|---|---|
| **지연** / Latency | **3-10ms** | 1-3ms (지원 시) | 2-5ms | **2-5ms** |
...
- **ASIO**: 장치가 지원하는 값만 표시. ASIO Control Panel에서 설정 가능 / Shows device-supported values only. Configurable via ASIO Control Panel
...
| 4 | **Windows** | 설정 → 시스템 → 사운드 → 출력/입력 장치 → 속성 → 고급 → 기본 형식 / Settings → System → Sound → Output/Input device → Properties → Advanced → Default Format |
...
- 스캔 로그: `%AppData%/DirectPipe/scanner-log.txt` / Scan log: `%AppData%/DirectPipe/scanner-log.txt`
...
- **Enable VST Receiver Output** — IPC 출력 켜기/끄기 (공유 메모리로 Receiver VST2에 오디오 전송) / Toggle IPC output on/off (sends audio to Receiver VST2 via shared memory)
- 기본값은 **꺼짐(OFF)**. OBS에서 Receiver VST2를 사용할 때만 켜면 됩니다 / Default is **OFF**. Only enable when using Receiver VST2 in OBS
...
| **Open Folder** | 녹음 폴더를 탐색기에서 열기 / Open recording folder in Explorer |
...
| **메뉴** / Menu | 트레이 아이콘 우클릭 → Show / Start with Windows / Quit / Right-click tray icon |
| **완전 종료** / Quit | 우클릭 → "Quit DirectPipe" / Right-click → "Quit DirectPipe" |
...
트레이 메뉴 또는 Settings 탭에서 설정. Windows 시작 시 트레이에서 자동 실행됩니다. / Toggle via tray menu or Settings tab. Auto-starts in system tray on Windows startup.
...
|---|---|---|
| **OUT** | DirectPipe → VB-Cable → Discord | 음성 통화 마이크 / Voice chat mic |
...
|---|---|---|
| **OUT** (Discord) | Output 탭 → Master Volume | 70% — 통화 상대에게 적당한 볼륨 / Comfortable volume for chat partner |
...
|---|---|
| Ctrl+Shift+1~9 | 플러그인 1-9 Bypass 토글 / Plugin 1-9 bypass toggle |
| Ctrl+Shift+0 | 마스터 Bypass (전체 체인) / Master bypass (entire chain) |
| Ctrl+Shift+M | 패닉 뮤트 / Panic mute |
| Ctrl+Shift+N | 입력 뮤트 토글 / Input mute toggle |
| Ctrl+Shift+O | 출력 뮤트 토글 / Output mute toggle |
| Ctrl+Shift+H | 모니터 토글 / Monitor toggle |
| Ctrl+Shift+I | IPC 출력 토글 / IPC output toggle |
...
- **Start with Windows** — Windows 시작 시 트레이에서 자동 실행 / Auto-start in system tray on Windows startup
...
- **Save Settings** → `.dpbackup` 파일로 저장 (설정만) / Save to `.dpbackup` file (settings only)
- **Load Settings** → `.dpbackup` 파일에서 복원 (설정만) / Restore from `.dpbackup` file (settings only)
...
|---|---|
| **Full Backup** | 전체 백업 (설정 + VST 체인 + 슬롯 A-E + 컨트롤) → `.dpfullbackup` 파일 / Full backup (settings + VST chain + slots A-E + controls) → `.dpfullbackup` file |
| **Full Restore** | `.dpfullbackup` 파일에서 전체 복원 / Restore everything from `.dpfullbackup` file |
...
3. 미터가 안 움직이면 / If meter doesn't move:
   - Windows 설정 → 개인 정보 → 마이크 접근 허용 확인 / Windows Settings → Privacy → allow microphone access
   - 다른 앱의 독점 모드 해제 / Disable exclusive mode in other apps
...
3. 오디오 인터페이스가 있다면 **ASIO** 드라이버 사용 / Use **ASIO** driver if you have an audio interface
...
5. **Windows Audio (Low Latency)** 에서 끊기면 → **Windows Audio**로 변경 시도 (LL 모드가 제대로 지원되지 않는 장치가 많음) / If crackling on **Low Latency** → try **Windows Audio** (many devices don't properly support LL mode)
...
4. 모니터는 별도 WASAPI 장치 — ASIO 모드에서도 동작 / Monitor uses separate WASAPI device — works even with ASIO
...
- 로그 / Log: `%AppData%/DirectPipe/scanner-log.txt`

### Receiver VST2에서 끊김 / Receiver audio crackling

1. Receiver VST GUI에서 **Buffer** 한 단계 올리기 / Increase **Buffer** one step in Receiver VST GUI
...
| Windows 마이크 "이 장치로 듣기" 활성화 / Windows mic "Listen to this device" enabled | Windows 사운드 설정 → 마이크 속성 → "수신 대기" 탭 → 체크 해제 / Windows Sound → mic properties → "Listen" tab → uncheck |
| VB-Cable 출력이 DirectPipe 입력에 연결 / VB-Cable output connected to DirectPipe input | Audio 탭 Input이 **실제 마이크**인지 확인 (VB-Cable Output이 아님) / Verify Audio tab Input is your **real mic** (not VB-Cable Output) |
...
| 파일 / File | 경로 / Path |
|---|---|
| 기본 설정 / Main settings | `%AppData%/DirectPipe/settings.dppreset` |
| 프리셋 슬롯 / Preset slots | `%AppData%/DirectPipe/Slots/` |
| 플러그인 캐시 / Plugin cache | `%AppData%/DirectPipe/plugin-cache.xml` |
| 스캐너 로그 / Scanner log | `%AppData%/DirectPipe/scanner-log.txt` |
| 녹음 폴더 / Recording folder | `Documents/DirectPipe Recordings/` |
| 포터블 모드 / Portable mode | `./config/` (exe 옆 `portable.flag` 필요 / requires `portable.flag` next to exe) |
