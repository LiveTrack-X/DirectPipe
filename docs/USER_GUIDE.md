...
> **Current Release 4.2.7 / 현재 릴리즈 4.2.7** — [Download the public release / 공개 릴리즈 다운로드](https://github.com/LiveTrack-X/DirectPipe/releases/tag/v4.2.7).
...
> **플랫폼 지원 / Platform Support**: Windows 10/11 x64 (정식/안정 대상), macOS 10.15+ universal (베타), Linux x86_64 (실험적). v4.2.7은 전체 Windows Release 검증과 정확 태그 CI run `30571612012`의 전체 플랫폼 빌드·등록 테스트·패키지 checksum·실행 파일 버전 신원을 통과해 게시됐습니다. 인증서가 없으므로 Windows 패키지는 unsigned입니다. 실기기·제3자 VST crash-containment와 macOS/Linux 하드웨어 동작은 별도 검증 범위입니다.
...
> **Platform Support**: Windows 10/11 x64 (stable target), macOS 10.15+ universal (beta), Linux x86_64 (experimental). v4.2.7 was published after the complete Windows Release suite and exact-tag CI run `30571612012` passed cross-platform builds, registered tests, package checksums, and executable-version verification. Without a certificate Windows is unsigned. Real-device, third-party VST crash-containment, and macOS/Linux hardware behavior remain separate evidence.
...
>
> **ASIO restore behavior (Windows)**: ASIO input/output restore as one duplex ASIO device. If the saved ASIO device is not visible at startup, DirectPipe waits for the saved device instead of opening another ASIO driver such as FL Studio ASIO or Realtek ASIO.
...
| 버퍼 / Buffer | 한 버퍼 주기 (@48kHz) / One buffer period | 권장 / Recommended |
|---|---|---|
...
- **첫 클릭**: Filter + Noise Removal + Auto Gain(내부 Post Limiter 포함) 기본 체인 생성 / First click: creates default chain with Auto Gain (including internal post limiter)
...
| **Auto Gain** | LUFS 기반 자동 볼륨 레벨러 + 고정 Post Limiter | Target -15 LUFS, Max Gain 22 dB, Limiter Ceiling -1.0 dBTP |
...
- **Limiter Ceiling**: Post limiter ceiling (고급에서 노출되는 유일한 limiter 항목, -2.0~0.0 dBTP, 기본 -1.0)
- **고정 내부값 / Fixed internal**: post limiter lookahead 1ms, release 50ms (UI 미노출)

> **참고**: 48kHz가 아닌 샘플레이트에서는 Noise Removal이 비활성화됩니다. 내부 리샘플링은 `DP-NOISE-RESAMPLE-0001`로 보류되어 있습니다.
> **Note**: Noise Removal is inactive at non-48kHz sample rates. Internal resampling is deferred under `DP-NOISE-RESAMPLE-0001`.
...
> For transient-heavy chains (e.g., OBS/streaming), keep Safety Guard enabled with conservative ceiling headroom.
...
- **Windows**: 레지스트리 (`HKCU\...\Run`)를 통한 시작 프로그램 등록. 실행 파일 경로는 공백이 있는 설치 경로에서도 동작하도록 따옴표로 저장됩니다. / Registry-based startup entry. The executable path is quoted so startup works from install paths with spaces.
...
- 패닉 뮤트 중에는 OUT/MON/VST 버튼이 잠기고 대부분 외부 제어 액션이 차단됨 (예외: Input Mute/XRun Reset/Safety Guard(legacy SafetyLimiter actions)/Auto Processors Add)
- **녹음 중이면 자동 중지** — 마이크 관련 출력이 전부 차단되므로 녹음도 함께 중지
- 해제하면 이전 ON/OFF 상태가 **자동 복원** (예: VST만 OFF였다면 그 상태로 돌아감. 단, 녹음은 자동 재시작 안 함)
- During panic mute, OUT/MON/VST buttons are locked and most external-control actions are blocked (exceptions: Input Mute/XRun Reset/Safety Guard (legacy SafetyLimiter actions)/Auto Processors Add)
...
- **개별 Bypass / Individual Bypass**: 플러그인 행의 Bypass 토글 또는 기본 단축키 **Ctrl+Shift+1~3**(Controls 탭에서 추가 가능) / Bypass toggle on plugin row or default hotkeys **Ctrl+Shift+1~3** (more can be added in Controls)
...
> **알려진 제한 / Known limitation**: 개별 플러그인 Bypass는 그래프에서 해당 노드를 우회하므로 리버브·딜레이 잔향이 즉시 끊깁니다. 자연스러운 tail 처리 방식은 `DP-VST-BYPASS-TAIL-0001`로 보류되어 있습니다. / Individual plugin bypass routes around the graph node, so reverb and delay tails stop immediately. Natural tail handling is deferred under `DP-VST-BYPASS-TAIL-0001`.
...
- **Auto Start** — 시스템 시작 시 트레이/메뉴 바에서 자동 실행 / Auto-start in system tray/menu bar on system startup (Windows: quoted registry Run command, macOS: Launch Agent, Linux: XDG autostart)
...
4. 장치가 목록에는 보이지만 입력/출력 채널이 잡히지 않으면 DirectPipe가 드라이버 기본 채널로 자동 재시도하고 로그에 `activeIn`/`activeOut`을 남깁니다. 문제 신고 시 로그를 함께 확인하세요 / If a device is listed but active input/output channels are not available, DirectPipe retries driver default channels and logs `activeIn`/`activeOut`.
5. **OUT** 버튼이 초록색인지 확인 (빨간색 = 뮤트) / Check **OUT** button is green (red = muted)
6. **PANIC MUTE** 활성화 여부 확인 / Check if **PANIC MUTE** is active
