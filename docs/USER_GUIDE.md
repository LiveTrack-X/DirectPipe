...
### 세이프티 리미터 / Safety Limiter

세이프티 리미터는 **출력 직전 전역(stage-last)** 에 적용되는 보호 장치입니다.  
Safety Limiter is a **global stage-last** protection block applied right before output delivery.

- 적용 경로 / Applied paths: OUT, MON, VST(IPC), REC (WAV)
- 동작 방식 / Behavior: 2ms look-ahead + instant attack + smooth release + hard sample clamp (brickwall style)
...
> For transient-heavy chains (e.g., OBS/streaming), keep Safety Limiter enabled with conservative ceiling headroom.
