...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/version-4.0.0-4fc3f7?style=flat-square" alt="Version">
...
- **3가지 출력 경로** — Main Output (Audio 탭 장치) + Monitor Output (Output 탭, 별도 오디오 장치로 헤드폰 모니터링) + VST Output (Receiver VST2 → OBS) — Three output paths: main, monitor headphones (via separate audio device), VST output to OBS
...
> **Tip**: 가상 케이블과 Receiver VST를 **동시에** 사용할 수도 있습니다. Discord는 가상 케이블로, OBS는 Receiver VST로 각각 보내면 됩니다. — You can use both methods simultaneously: virtual cable for Discord, Receiver VST for OBS.
...
보통 DirectPipe에서 처리된 오디오를 OBS로 보내려면 가상 오디오 케이블 (VB-Cable, BlackHole 등)이 필요합니다. Receiver VST2를 사용하면 **가상 케이블 없이** 공유 메모리(IPC)를 통해 오디오를 직접 받을 수 있어 설정이 더 간단하고 지연도 적습니다.
...
Normally, to route DirectPipe's processed audio to OBS, you need a **virtual audio cable** (VB-Cable, BlackHole, etc.). With the Receiver VST2, you can receive audio directly via shared memory (IPC) — **no virtual cable needed**, simpler setup, lower latency.

**Receiver VST2 vs. Virtual Cable 비교 / Comparison:**

| | Receiver VST2 | 가상 오디오 케이블 / Virtual Audio Cable |
|---|---|---|
| 설치 / Install | Receiver 플러그인을 VST 폴더에 복사 (.dll/.vst/.so) / Copy plugin to VST folder | 가상 케이블 설치 (VB-Cable/BlackHole 등) / Install virtual cable |
...
- 이렇게 하면 Discord에는 가상 케이블로, OBS에는 Receiver VST로 동시에 오디오를 보낼 수 있습니다
...
- This lets you send audio to Discord via virtual cable and to OBS via Receiver VST simultaneously
</details>
