...
MainComponent::MainComponent(bool enableExternalControls)
{
    ...
    settingsAutosaver_->onShowWindow = [safeThis = juce::Component::SafePointer<MainComponent>(this)] {
        ...
        if (auto* w = safeThis->getTopLevelComponent()) {
            // Startup-only behavior: if enabled, first show path routes to tray/minimized state.
            if (safeThis->isStartMinimizedToTrayOnLaunchEnabled()) {
                if (auto* docWindow = dynamic_cast<juce::DocumentWindow*>(w))
                    docWindow->closeButtonPressed();
                else
                    w->setVisible(false);
                return;
            }
...
{
    if (startMinimizedToTrayOnLaunch_ == enabled)
        return;

    startMinimizedToTrayOnLaunch_ = enabled;
    syncStartMinimizedToTrayToggle();

    if (markDirty)
        markSettingsDirty();
}

void MainComponent::syncStartMinimizedToTrayToggle()
{
