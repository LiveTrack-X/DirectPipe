...
/**
 * @file ipc-toggle.js
 * @brief IPC output toggle action — SingletonAction (@elgato/streamdeck SDK).
 */

const { SingletonAction } = require("@elgato/streamdeck");

class IpcToggleAction extends SingletonAction {
    manifestId = "com.directpipe.directpipe.ipc-toggle";

    onKeyDown(ev) {
        const { dpClient } = require("../plugin");
        dpClient.sendAction("ipc_toggle");
    }
    ...
    _updateDisplay(action, state) {
        ...
        action.setTitle(enabled ? "IPC ON" : "IPC OFF");
    }
}

IpcToggleAction.UUID = "com.directpipe.directpipe.ipc-toggle";

module.exports = { IpcToggleAction };
