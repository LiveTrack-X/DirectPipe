...
- **ActionHandler**: Consolidated action routing from ActionDispatcher to AudioEngine/UI. `doPanicMute(bool)` consolidates panic mute logic (single implementation, was scattered in v3 MainComponent): saves pre-mute state (monitor, output, IPC), blocks output paths, **stops active recording**. On unmute, restores previous state (recording does not auto-restart). Most action cases in `handle()` check `engine_.isMuted()` to block during panic — PluginBypass, MasterBypass, InputGainAdjust, SetVolume, SetPluginParameter, LoadPreset, SwitchPresetSlot, NextPreset, PreviousPreset, RecordingToggle, MonitorToggle, IpcToggle. Guard-bypass actions are PanicMute/InputMuteToggle/XRunReset/SafetyLimiterToggle/SetSafetyLimiterCeiling/AutoProcessorsAdd. `AutoProcessorsAdd`가 panic 중 허용되는 이유: 방송 중 panic 상태에서 Auto 체인을 미리 준비해두고 해제 후 바로 사용하는 시나리오를 지원. 체인 구조 변경이지만 panic 중 출력이 차단되어 있으므로 실제 오디오에 영향 없음. SafetyLimiterToggle/XRunReset과 같은 "준비/유지보수" 액션 범주. Callback-based decoupling from MainComponent (`onDirty`, `onNotification`, `onPanicStateChanged`, `onRecordingStopped`, etc.). Owns pre-mute state (`preMuteMonitorEnabled_`, `preMuteOutputMuted_`, `preMuteVstEnabled_`).
...
- **Panic Mute**: Remembers pre-mute state (monitor, output mute, IPC), restores on unmute. Stops active recording on engage (does not auto-restart). During panic mute, output-path controls (OUT/MON/VST) are locked and blocked while panic is active. Logic consolidated in `ActionHandler::doPanicMute()`. `input_muted` is independent from `muted` (`InputMuteToggle` mutes input only and keeps chain/output paths running).
