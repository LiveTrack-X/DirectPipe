...
### Audio
- **Built-in Processors**: BuiltinFilter (HPF+LPF), BuiltinNoiseRemoval (RNNoise+FIFO+VAD), BuiltinAutoGain (LUFS+Leveler). All inherit AudioProcessor, inserted into AudioProcessorGraph alongside VSTs. [Auto] button adds all 3. Auto preset slot (index 5, non-renameable, rotation-excluded).
...
- RNNoise: BSD-3 (thirdparty/rnnoise/), 48kHz fixed, 480-frame FIFO (~10ms latency), dual-mono
