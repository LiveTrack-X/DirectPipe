...
### Audio
- **Built-in Processors**: BuiltinFilter (HPF+LPF), BuiltinNoiseRemoval (RNNoise+FIFO+VAD), BuiltinAutoGain (LUFS+Leveler). All inherit AudioProcessor, inserted into AudioProcessorGraph alongside VSTs. [Auto] button = special preset slot (index 5, non-renameable, rotation-excluded) next to input gain; right-click for Reset to Defaults. **RNNoise**: requires 32767 scaling (int16 range), 2-pass FIFO for in-place safety, ring buffer output, gate starts closed, 5-frame warmup before VAD active. **AGC**: Freeze Level uses per-block RMS gate (not LUFS), default -45 dBFS. Attack 500ms, Release 700ms, LUFS window 1.5s. Max Gain default 24 dB. Correction % = envelope speed (not gain amount). **VAD thresholds**: Light 0.50, Standard 0.70, Aggressive 0.90.
...
- RNNoise: BSD-3 (thirdparty/rnnoise/), 48kHz fixed, 480-frame FIFO (~10ms latency), dual-mono. Requires 32767 scaling (int16 range), 2-pass FIFO for in-place safety, ring buffer output, gate starts closed, 5-frame warmup
- AGC defaults: target -15 LUFS, attack 500ms, release 700ms, LUFS window 1.5s, max gain 24 dB, freeze level -45 dBFS (per-block RMS gate, not LUFS). Correction % = envelope speed
- VAD thresholds: Light 0.50, Standard 0.70, Aggressive 0.90
